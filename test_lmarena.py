...
from playwright.async_api import async_playwright, Locator, Page, expect, TimeoutError as PlaywrightTimeoutError
import re
# 需要安装 lxml
...
async def main():
    async with async_playwright() as p:
        ...
        if not page.url.startswith("https://lmarena.ai"):
        ...
        bot = ChatBot(
            page,
            input_selector='',
            assistant_bubble_selector=r"div.no-scrollbar.relative.flex.w-full.flex-1.flex-col.overflow-x-auto.transition-\[max-height\].duration-300"
        )
